...
- R3. 신청 페이지 드롭다운은 페이지에 고정(24개, §9-1). 공개 API `GET /api/public/programs`는 전체 프로그램을 반환하되 항목마다 `status`·`open`·`reason`·`label`·`period`를 붙인다(`lib/assign.ts:applyAvailability`). 접수 가능 = 상태 「진행중」인 프로그램(대여기간 입력 여부와 무관). 시작전·완료는 사유와 함께 비활성화한다.
  | 프로그램 상태 | 대여기간 | 드롭다운 표시 | 선택 |
  |---|---|---|---|
  | 시작전 | 무관 | `이름 — 접수 준비 중` | ✕ |
  | 진행중 | 설정 | `이름 — 접수중` + 대여기간 표시 | ○ |
  | 진행중 | 미설정 | `이름 — 접수중` + 「대여기간 추후 안내(배정 시 확정)」 | ○ |
  | 완료 | 무관 | `이름 — 접수 종료` | ✕ |
  | 미등록 | — | `이름 — 미등록` | ✕ |

  대여기간 미설정 상태로 접수된 건은 `rent_start`·`rent_end` 가 비어 있고, 관리자가 기간을 입력할 때까지 자동 배정(R14·R15) 대상에서 제외된다(접수만 받고 배정은 보류).
...
| 프로그램 | 목록(배정가능 GPT/Claude, 배정 현황/상한), 등록·수정(프로그램ID 드롭다운 — 기존 선택 시 값 자동 로드, 새 프로그램은 저장 시 자동 채번 R1; 이름·대상·방식·기간·상한·상태·비고), 「하반기 24개 일괄 등록」, 종료 처리 |
